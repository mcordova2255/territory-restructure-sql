...
Changes can only be raised inside a deployment window. Sequence matters more than speed, because a correction raised against the wrong window lands in the wrong half and is measured against the wrong quota period.
The book has to reconcile. Closing value must equal opening value plus the movement the cases caused plus the movement the forecast refresh caused. If it does not tie, value went missing in the restructure and the whole exercise is indefensible.
